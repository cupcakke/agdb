...
const DeleteError = error{ GetdentsFailed, UnlinkatFailed, PathTooLong };

fn deleteDirContents(dir_fd: i32) DeleteError!void {
    ...
    while (true) {
        ...
        while (pos < limit) {
            ...
            if (!std.mem.eql(u8, name_slice, ".") and !std.mem.eql(u8, name_slice, "..")) {
                recursiveDeleteAt(dir_fd, name_ptr) catch {};
            }
            pos = next_pos;
        }
    }
...
fn recursiveDeleteAt(parent_fd: i32, name_z: [*:0]const u8) DeleteError!void {
    ...
    if (fd_err == .SUCCESS) {
        ...
        const rc = std.os.linux.unlinkat(parent_fd, name_z, AT_REMOVEDIR);
        if (std.posix.errno(rc) != .SUCCESS) return error.UnlinkatFailed;
    } else {
        const rc = std.os.linux.unlinkat(parent_fd, name_z, 0);
        if (std.posix.errno(rc) != .SUCCESS) return error.UnlinkatFailed;
    }
}

fn recursiveDelete(path: []const u8) DeleteError!void {
    ...
    try recursiveDeleteAt(AT_FDCWD, path_z);
}
