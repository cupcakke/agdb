...
pub const SecurityManager = struct {
    ...
    pub const PoolOp = enum { read, write, allocate, deallocate, set_root };

    pub fn attach(self: *Self, pool_uuid: u128) !void {
        if (!self.enabled) return;
        var salt: [16]u8 = undefined;
        std.mem.writeInt(u128, &salt, pool_uuid, .little);
        const key_id: u64 = @truncate(pool_uuid);
        _ = try self.deriveKey(&salt, "pool-attach", key_id);
    }

    pub fn detach(self: *Self, pool_uuid: u128) !void {
        const key_id: u64 = @truncate(pool_uuid);
        self.lock.lock();
        defer self.lock.unlock();
        if (self.region_keys.fetchRemove(key_id)) |entry| {
            var key = entry.value;
            crypto.utils.secureZero(u8, &key.data);
        }
    }

    pub fn check(self: *Self, pool_uuid: u128, op: PoolOp) !void {
        _ = op;
        if (!self.enabled) return;
        const key_id: u64 = @truncate(pool_uuid);
        self.lock.lock();
        defer self.lock.unlock();
        if (self.region_keys.get(key_id) == null) return error.PoolNotAttached;
    }
};
