...
pub const Server = struct {
    ...
    pub fn run(self: *Server) !void {
        ...
        while (self.running.load(.acquire)) {
            ...
            const t = std.Thread.spawn(.{}, connThread, .{ctx}) catch |spawn_err| {
                std.log.warn("agdb server: spawn error: {s}", .{@errorName(spawn_err)});
                conn.stream.close();
    ...
    fn connThread(ctx: *ConnCtx) void {
        ...
        ctx.server.handleConnection(ctx.conn) catch |err| {
