...
pub const RecoveryEngine = struct {
    ...
    fn needsRecoveryChecked(self: *Self) !bool {
        if (self.heap.heapHeaderMut().isDirty()) {
            return true;
        }
            ...
            .heap_extend => {
                if (record.size > 0) {
                    ...
                    if (current_size < target_size) {
                        const was_active = self.heap.active_transaction;
                        self.heap.active_transaction = true;
                        defer self.heap.active_transaction = was_active;
    ...
    fn undoAllocate(self: *Self, record: *const wal_mod.WALRecord) !void {
        ...
        obj_header.checksum = obj_header.computeChecksum();
        try self.heap.flushRangeAt(record.offset, @sizeOf(header.ObjectHeader));
        if (!try self.heap.isInFreeList(record.offset, record.size)) {
            try self.heap.freeListInsert(record.offset, record.size);
            try self.heap.flushFreeListMetadata();
        }
        try self.heap.markFreed(record.offset, record.size);
    ...
    fn finalizeRecovery(self: *Self) !void {
        ...
        errdefer {
            if (!dirty_restored) {
                self.heap.heapHeaderMut().setDirty(true);
                self.heap.heapHeaderMut().checksum = 0;
                self.heap.heapHeaderMut().updateChecksum();
        ...
        self.heap.heapHeaderMut().setDirty(false);
        self.heap.heapHeaderMut().checksum = 0;
        self.heap.heapHeaderMut().updateChecksum();
        try self.heap.flushRangeAt(0, @sizeOf(header.HeapHeader));
        try self.heap.sync();
    ...
    fn scanHeap(self: *Self, comptime repair: bool) !if (repair) usize else bool {
        ...
        if (repair) {
            const heap_header_expected = self.heap.heapHeaderMut().computeChecksum();
            if (self.heap.heapHeaderMut().checksum != heap_header_expected) {
                const can_repair = self.heap.heapHeaderMut().canRepair() catch false;
                if (can_repair) {
                    self.heap.heapHeaderMut().checksum = 0;
                    self.heap.heapHeaderMut().updateChecksum();
                    try self.heap.flushRangeAt(0, @sizeOf(header.HeapHeader));
            ...
            self.heap.heapHeaderMut().validate() catch {
                result = false;
            };
        }
